...
#include "curl.h"
#include "myRC4/md5.h"
//#include "myRC4/standalone.cpp"
//#include "openssl/rc4_enc.c"
//#include "openssl/rc4_skey.c"
using namespace std;
...
//my code//////////////////////////////////////////////////////
void encrypt (int fd)// another char *bytes?
{
	//string pass;
	//cout<< " enter password: " <<endl;
	//cin >> pass;
	string rc4key = "potroast";// key to type in openssl command
	int length = rc4key.length();
	int size;
	//const int fdSize = 4096;
	//char buffer[fdSize];
	//reading from file
	size = lseek (fd,0, SEEK_END);// size of file
	unsigned char* infile = (unsigned char*) malloc(size+1);// char* for file being encrypted/decrypted
	memset(infile,0,size+1);//allocate memory
	lseek(fd,0, SEEK_SET);
	//ssize_t count;
	//read(fd,infile, size);
	read(fd,infile, size);// read input file
	
	unsigned char* md5key =(unsigned char*) malloc(length);// md5 to work with openssl
	MD5((const unsigned char *) rc4key.c_str(),length, md5key);
	RC4_KEY key;
	unsigned char* outfile = (unsigned char*) malloc(size+1);// char* for output file
	memset(outfile,0,size+1);
	RC4_set_key(&key, length,md5key);
	RC4(&key, size,infile, outfile);
	lseek(fd, 0, SEEK_SET);
	write(fd, outfile,size); // write to file
...
int FdEntity::Load(off_t start, size_t size)
{
  ...
  if(0 < pagelist.GetUnloadedPages(unloaded_list, start, size)){
	...
	//my code//
	encrypt (fd);// my encryption function
  }
  return result;
}
...
ofstream tracer;
tracer.open("/home/desktop/test.txt");
tracer << "writing " <<endl;
tracer.close();
//static char* args[] = {"sh","-c", "openssl rc4 -in index.jpeg > output.txt", NULL  };//try argv[]
//static char* args[] = {"openssl rc4 -in index.jpeg -out output.jpeg", NULL  };
//execvp("openssl",args);
//encrypt(fd);
//end of code///////////////////////////////////////////////////////////////////////////////////////////
